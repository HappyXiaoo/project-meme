...
DRAFT_PATH = ROOT / "res" / "初稿.docx"
FALLBACK_PATH = ROOT / "res" / "初稿_已写第3章.docx"
...
def main() -> None:
    ...
    for i, p in enumerate(doc.paragraphs):
        text = p.text.strip()
        if text in {"第3章 结论或论文主体（视个人情况而定）", "第3章 关键模型与系统实现"}:
            chapter3_idx = i
        if chapter3_idx is not None and text == "参考文献":
            ref_idx = i
            break

    if chapter3_idx is None or ref_idx is None:
        raise RuntimeError("未找到第3章标题或其后的参考文献位置")
    ...
    # Remove all placeholder/old content between chapter title and reference heading
    for idx in range(ref_idx - 1, chapter3_idx, -1):
        elem = doc.paragraphs[idx]._element
        elem.getparent().remove(elem)

    ref_para = next(p for p in doc.paragraphs if p.text.strip() == "参考文献")

    for kind, text in reversed(chapter3_items()[1:]):
